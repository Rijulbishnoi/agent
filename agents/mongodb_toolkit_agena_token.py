# mongodb_toolkit_agen.py - Final Working Version with Per-LLM-Call Token Counting for Bug Testing
import os
import json
...
# NEW: Custom LLM wrapper to count tokens on EVERY LLM call
class TokenCountingLLMWrapper(ChatGoogleGenerativeAI):
    def _generate(self, messages: List, stop: Optional[List[str]] = None, **kwargs: Any) -> LLMResult:
        # NEW: Count input tokens (prompt)
        input_text = " ".join([msg.content for msg in messages])  # Concatenate messages for counting
        input_token_count = genai.GenerativeModel('gemini-1.5-flash').count_tokens(input_text).total_tokens
        
        # Generate as normal
        result = super()._generate(messages, stop, **kwargs)
        
        # NEW: Count output tokens (generated text)
        output_text = " ".join([gen["text"] for gen in result.generations[0]])  # Assuming single generation
        output_token_count = genai.GenerativeModel('gemini-1.5-flash').count_tokens(output_text).total_tokens
        
        # NEW: Log and store per call (with unique call_id)
        call_id = str(uuid.uuid4())
        request_id = current_request_context.get("request_id")  # Assume global context for request_id (set in executor)
        user_id = current_user_context.get("user_id")
        user_company = get_user_company(user_id)  # Synchronous version for simplicity; make async if needed
        ...
        log_entry = {
            ...
            "total_tokens_per_call": input_token_count + output_token_count,
            "input_snippet": input_text[:500],
            "output_snippet": output_text[:500],
            "timestamp": datetime.now()
        }
        db.token_usage_logs.insert_one(log_entry)  # Synchronous for simplicity; use await if in async context
        
        logger.info(f"Per-LLM-call token usage (call_id: {call_id}): input={input_token_count}, output={output_token_count}")
        
        return result

# Initialize wrapped LLM
llm = TokenCountingLLMWrapper(
...
# Helper function to get user company (made synchronous for wrapper compatibility; use async in tools if needed)
def get_user_company(user_id: str):
    """Get user's company ID - CRITICAL for company filtering (synchronous version)"""
    if not user_id or not ObjectId.is_valid(user_id):
        logger.warning(f"Invalid user_id: {user_id}")
        return None
    
    try:
        user = db.users.find_one({"_id": ObjectId(user_id)})  # Synchronous DB call (use await in async contexts)
        if not user:
...
class MongoDBQueryTool(BaseTool):
    ...
    async def _arun(
        ...
        try:
            ...
            user_company = get_user_company(user_id)  # Now synchronous
            if not user_company:
                return {
                    "status": "error",
...
# Token Count Tool (enhanced with storage for bug testing)
class TokenCountInput(BaseModel):
    text: str = Field(description="The text to count tokens for")
    is_input: Optional[bool] = Field(description="Whether this is input (True) or output (False)", default=True)
    request_id: Optional[str] = Field(description="Unique ID for the request (for logging)", default=None)  # NEW: For tracking per request


class TokenCountTool(BaseTool):
    name: str = "token_count"
    description: str = "Count the number of input or output tokens for Gemini LLM to monitor API usage."
    args_schema: Type[BaseModel] = TokenCountInput


    def _run(self, text: str, is_input: bool = True, request_id: Optional[str] = None) -> str:
        return "This tool only supports async operations"


    async def _arun(self, text: str, is_input: bool = True, request_id: Optional[str] = None) -> dict:
        try:
            # Count tokens
            ...
            result = {
                ...
                "message": f"{'Input' if is_input else 'Output'} text has {token_count} tokens.",
                "request_id": request_id or str(uuid.uuid4())  # NEW: Ensure unique ID
            }
            
            # NEW: Store in MongoDB for bug testing and per-request tracking (company-filtered)
            user_id = current_user_context.get("user_id")
            user_company = get_user_company(user_id)  # Synchronous call
            log_entry = {
                ...
                "token_count": token_count,
                "text_snippet": text[:500],  # Limit for efficiency
                "timestamp": datetime.now()
            }
            db.token_usage_logs.insert_one(log_entry)  # Synchronous insert (fine for logging)
            ...
            return {
                "status": "error",
                "message": f"Token counting failed: {str(e)}"
            }
...
- Token monitoring: Use token_count to count input/output tokens for any text
...
Action: [mongodb_query OR mongodb_aggregation OR mongodb_get_user_permissions OR token_count]
...
current_request_context = {"request_id": None}  # NEW: For tracking request_id in LLM wrapper


# Enhanced agent executor with AUTOMATIC per-request token counting
class CustomAgentExecutor(AgentExecutor):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        object.__setattr__(self, 'token_tool', TokenCountTool())  # FIXED: Bypass Pydantic setter to avoid ValueError
    
    async def ainvoke(self, inputs, config=None):
        global current_user_context, current_request_context
        ...
        current_request_context["request_id"] = str(uuid.uuid4())  # NEW: Set request_id for this execution
        logger.info(f"User context set for validated user: {user_id}")
        logger.info(f"Processing request ID: {current_request_context['request_id']}")
        
        try:
            result = await super().ainvoke(inputs, config)
            
            # Aggregate per-request totals from logs (or keep as-is; per-call is already stored)
            # For simplicity, we can query the logs for this request_id if needed, but here we add a summary
            token_usage = {
                "request_id": current_request_context["request_id"],
                "note": "Per-call counts stored in token_usage_logs. Query for details."
            }
            ...
            current_request_context["request_id"] = None
...
logger.info("HomeLead MongoDB Toolkit Agent initialized successfully with per-LLM-call token counting")
